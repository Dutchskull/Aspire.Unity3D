...
public sealed class UnityControlClient : IDisposable
{
    ...
    public async Task<bool> PassEnvironmentVariablesAsync(Uri baseUrl, Dictionary<string, string> environmentsVariables, CancellationToken cancellationToken = default)
    {
        string relativePath = $"config";
        Uri url = new(baseUrl, relativePath);
        StringContent content = new(JsonSerializer.Serialize(environmentsVariables), Encoding.UTF8, "application/json");
        try
        {
